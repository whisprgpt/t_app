  ...
  const apply = () => {
    ...
    if (wizardState.template === "exams") {
      if (wizardState.subject === "other") {
        onPrompt(
          classSystemPrompt.other(wizardState.customClass ?? "Custom Subject")
        );
      } else {
              ...
              <div className="grid grid-cols-2 gap-2">
                {codingLanguages.map((lang) => (
                  <Button
                    key={lang.value}
                    variant="outline"
                    className={`justify-start p-3 bg-slate-800/60 border-slate-600/40 text-emerald-100 hover:bg-slate-700/60 rounded-xl transition-all duration-300 text-sm ${
                      isCoding && wizardState.language === lang.label
              ...
              <div className="grid grid-cols-2 gap-2">
                {subjectClasses.map((subject) => (
                  <Button
                    ...
                    variant="outline"
                    className={`justify-start p-3 bg-slate-800/60 border-slate-600/40 text-emerald-100 hover:bg-slate-700/60 rounded-xl transition-all duration-300 text-sm ${
                      wizardState.subject === subject.value
